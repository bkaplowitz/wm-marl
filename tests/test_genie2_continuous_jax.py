...
def test_genie2_cli_smoke_writes_expected_artifacts(tmp_path) -> None:
    ...
    assert summary["imagined_transitions"] == 20
    assert np.isfinite(summary["bridge_replay_mse"])
...
def test_genie2_cli_official_dmc_pixel_smoke_writes_genuine_provenance(
    tmp_path,
) -> None:
    ...
    assert summary["policy_source"] == "latent_policy_bridge"
    assert np.isfinite(summary["bridge_replay_mse"])
    assert np.isfinite(summary["real_env_bridged_return"])
