...
def resolve_configuration(args, run):
    ...
    for phase, command in commands.items():
        ...
        if phase == "train":
            ...
            expected.update(
                envs=4, episodes=128, seed_offset=100000, policy_mode="eval"
            )
        if actual != expected:
    ...
    for index in range(42):
        ...
        jobs.append(
            dict(
                ...
                wandb_final=f"{PROJECT_URL}/runs/{job_id(run)}-final128",
            )
        )
...
def main():
    ...
    parser.add_argument("mode", choices=("init", "check", "worker", "job", "status"))
