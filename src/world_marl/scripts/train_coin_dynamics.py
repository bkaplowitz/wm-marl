...
def parse_args() -> argparse.Namespace:
  ...
  if args.substrate != "coins":
    parser.error("world-marl-train-coin-dynamics currently targets --substrate coins")
...
def main() -> None:
  ...
  def on_train(step: int, row: dict[str, float]) -> None:
    ...
    if train_bar is not None:
      train_bar.set_postfix(loss=f"{row['loss']:.4g}", exact=f"{row['full_state_exact_accuracy']:.3f}")
      train_bar.update(1)
