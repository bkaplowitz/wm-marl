...
class CausalKVTransformer(nj.Module):
  ...
  def step(self, cache, pair, reset):
    ...
    pair = nn.cast(pair)
    cache = jax.tree.map(
        lambda x: nn.where(reset, jnp.zeros_like(x), x), cache)
    position = jnp.where(reset, 0, cache['position'] + 1)
    projected = self.sub(
        'pair_projection', nn.Linear, self.units, winit=self.winit)(pair)
    start = self.value(
        'start_token', nn.init('trunc_normal'), (self.units,), f32)
    ...
    for index in range(self.layers):
      with nj.scope(f'layer{index}'):
        ...
        qkv = self.sub(
            'qkv', nn.Linear, 3 * self.units, winit=self.winit)(normed)
        ...
        attended = self.sub(
            'attention_out', nn.Linear, self.units, winit=self.winit)(attended)
        x = residual + attended

        residual = x
        x = self.sub('ffn_norm', nn.Norm, self.norm)(x)
        x = self.sub(
            'ffn_in', nn.Linear, self.units * self.ffup, winit=self.winit)(x)
        x = nn.act(self.act)(x)
        x = self.sub(
            'ffn_out', nn.Linear, self.units, winit=self.winit)(x)
        x = residual + x
    ...
    deter = self.sub(
        'state_projection', nn.Linear, self.output, winit=self.winit)(x)
    next_cache = dict(
...
class TransformerRSSM(rssm.RSSM):
  ...
  def _observe(
      self, carry, tokens, action, reset, training, include_cache=False):
    del training
  ...
  def _cache(self, carry):
    return {
        key: carry[key] for key in ('keys', 'values', 'valid', 'position')}
