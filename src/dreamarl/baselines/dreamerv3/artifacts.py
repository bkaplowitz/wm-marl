...
def bin_episode_scores_by_count(
    scores: Iterable[EpisodeScore],
    *,
    bins: int = 30,
    max_steps: int,
) -> list[dict[str, Any]]:
    """Bin returns using the fixed-count rule used by DreamerV3 plots."""
    if bins < 1:
        raise ValueError("bins must be >= 1")
    if max_steps < 1:
        raise ValueError("max_steps must be >= 1")
    filtered = [score for score in scores if score.env_steps <= max_steps]
    edges = np.linspace(0.0, float(max_steps), bins + 1)
    steps = np.asarray([score.env_steps for score in filtered], dtype=np.float64)
    returns = np.asarray([score.episode_return for score in filtered], dtype=np.float64)
    sums = np.histogram(steps, bins=edges, weights=returns)[0]
    counts = np.histogram(steps, bins=edges)[0]
    means = np.divide(
        sums,
        counts,
    ...
    for index, count in enumerate(counts):
        ...
        lower = edges[index]
        upper = edges[index + 1]
        upper_mask = steps <= upper if index == bins - 1 else steps < upper
        bucket = returns[(steps >= lower) & upper_mask]
        rows.append(
            {
                "env_steps": int(round(upper)),
                "episode_return_mean": float(means[index]),
                "episode_return_std": float(np.std(bucket)),
                "episodes": int(count),
            }
        )
    return rows
...
def load_official_reference(
    ...
    *,
    ...
    if observation_mode not in {"proprio", "vision"}:
        raise ValueError("observation_mode must be either proprio or vision")
    suite = f"dmc_{observation_mode}"
    path = Path(upstream_root) / "scores" / f"{suite}-dreamerv3.json.gz"
    if not path.exists():
        return None
    ...
    trajectories = []
    for row in rows:
        row_xs = np.asarray(row["xs"], dtype=np.float64)
        row_ys = np.asarray(row["ys"], dtype=np.float64)
        if row_xs.ndim != 1 or row_ys.ndim != 1 or len(row_xs) != len(row_ys):
            raise ValueError(
                "official Dreamer reference rows require paired 1D xs and ys"
            )
        trajectories.append((row_xs, row_ys))

    first_xs = trajectories[0][0]
    aligned = all(
        len(row_xs) == len(first_xs) and np.array_equal(row_xs, first_xs)
        for row_xs, _ in trajectories
    )
    if aligned:
        xs = first_xs
        ys = np.stack([row_ys for _, row_ys in trajectories])
    else:
        lower = max(row_xs[0] for row_xs, _ in trajectories)
        upper = min(row_xs[-1] for row_xs, _ in trajectories)
        if upper <= lower:
            raise ValueError("official Dreamer reference curves do not overlap")
        points = min(len(row_xs) for row_xs, _ in trajectories)
        xs = np.linspace(lower, upper, points, dtype=np.float64)
        ys = np.stack(
            [np.interp(xs, row_xs, row_ys) for row_xs, row_ys in trajectories]
        )
    return {
        "task": task,
        "suite": suite,
        "source": f"danijar/dreamerv3 scores/{suite}-dreamerv3.json.gz",
        "seeds": [int(row["seed"]) for row in rows],
        "env_steps": xs.astype(int).tolist(),
        "mean": np.mean(ys, axis=0).tolist(),
        "std": np.std(ys, axis=0).tolist(),
        "per_seed": {
            str(row["seed"]): [float(value) for value in values]
            for row, values in zip(rows, ys, strict=True)
        },
    }


def load_official_binned_reference(
    upstream_root: str | Path,
    *,
    task: str,
    observation_mode: str = "proprio",
    ...
    """Reproduce DreamerV3's per-seed binning and seed aggregation."""
    if observation_mode not in {"proprio", "vision"}:
        raise ValueError("observation_mode must be either proprio or vision")
    suite = f"dmc_{observation_mode}"
    path = Path(upstream_root) / "scores" / f"{suite}-dreamerv3.json.gz"
    if not path.exists():
        return []
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        runs = [row for row in json.load(handle) if row.get("task") == task]

    per_seed: list[dict[int, float]] = []
    for run in runs:
        scores = [
            EpisodeScore(int(step), float(value))
            for step, value in zip(run["xs"], run["ys"], strict=True)
        ]
    ...
    common_steps = set.intersection(*(set(curve) for curve in per_seed))
    result = []
    for env_steps in sorted(common_steps):
        values = np.asarray(
            [curve[env_steps] for curve in per_seed],
            dtype=np.float64,
        )
        result.append(
            {
                "env_steps": env_steps,
                "episode_return_mean": float(np.mean(values)),
                "episode_return_std": float(np.std(values)),
                "seeds": len(values),
            }
        )
    return result
...
def normalize_training_artifacts(
    ...
    *,
    ...
    bin_size: int = 10_000,
    paper_bins: int = 30,
    observation_mode: str | None = "proprio",
    ...
    paper_curve = bin_episode_scores_by_count(
        scores,
        bins=paper_bins,
        max_steps=train_steps_budget,
    )
    reference = (
        ...
        if observation_mode is not None and task.startswith("dmc_")
        else None
    )
    ...
    summary = {
        ...
        "paper_curve_bins": paper_bins,
        "paper_curve_x_axis": "training environment transitions",
        "paper_curve_y_axis": "mean raw collection return within each bin",
        "official_reference_suite": reference["suite"] if reference else None,
    }
    _write_jsonl(
    ...
    _write_json(normalized_dir / "paper_training_curve.json", paper_curve)
    _write_json(normalized_dir / "training_summary.json", summary)
    if reference:
        _write_json(normalized_dir / "official_reference.json", reference)
    _plot_training_curve(normalized_dir / "training_curve.png", paper_curve, reference)
    return summary
