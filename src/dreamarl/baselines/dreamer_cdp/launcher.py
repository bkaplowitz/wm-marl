...
from dreamarl.baselines.dreamerv3.config import default_upstream_root as dreamer_root
...
def run_training(
    ...
    *,
    ...
    if spec.upstream_logdir.exists() and not resume:
        raise FileExistsError(f"upstream logdir already exists: {spec.upstream_logdir}")
