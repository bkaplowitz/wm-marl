...
import numpy as np
from .replay_codec import filter_transport


def _prepare_batch(agent, data, counter, offset=0):
    data = filter_transport(agent, data)
...
class ReplayUpdateStream:
    """Sample both replay views only when the serial learner requests an update.

    The caller first completes collection and replay insertion, then checks
    prefill and the update schedule. Each take reads the current replay in
    world/behavior order; it neither caches an early batch nor samples ahead.
    The two views retain their independent selectors and sampling RNGs.
    """

    def __init__(self, agent, source, on_snapshot=None, *, behavior_source):
        self.agent = agent
        self.source = iter(source)
        self.behavior_source = iter(behavior_source)
        self.on_snapshot = on_snapshot
    ...
    def take(self, environment_step):
        ...
        data = {**data, **{f'_behavior_replay/{key}': value
                          for key, value in other.items()}}
        if self.on_snapshot is not None:
            self.on_snapshot(int(self.agent.n_batches), int(environment_step), data)
        batch = _prepare_batch(self.agent, data, self.agent.n_batches)
        return batch, int(environment_step)
