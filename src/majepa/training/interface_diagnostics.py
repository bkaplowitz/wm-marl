...
def availability_metrics(logits, target, gated_mask, predicted_alive, actual_alive, valid):
    ...
    def mean(value, selected=weight):
        return (value.astype(jnp.float32) * selected).sum() / jnp.maximum(selected.sum(), 1)
    ...
    metrics = {
        ...
        "sampled_mask_expected_error": mean(jnp.where(target, 1 - sampled_probability, sampled_probability)),
        "sampled_mask_empty_probability": mean(empty_probability[..., 0], valid.astype(jnp.float32)),
    ...
    for name, sl in (("noop", slice(0, 1)), ("stop", slice(1, 2)),
                     ("move", slice(2, 6)), ("target", slice(6, None))):
        selected = weight[..., sl]
        metrics[f"raw_mask_{name}_nll"] = mean(nll[..., sl], selected)
        metrics[f"raw_mask_{name}_brier"] = mean((probability[..., sl] - label[..., sl]) ** 2, selected)
        metrics[f"raw_mask_{name}_count"] = selected.sum()
    for index in range(10):
        selected = weight * ((probability >= index / 10) &
