"""Optional last-step supervision of the deployed recurrent CTDE simulator.

The factual one-step objective is unchanged. Recorded actions determine the
targets even when a predicted mask would exclude them. Each transition uses
inference-mode joint dynamics and a sampled local posterior; state is detached
between transitions. Only the current joint prediction and its outcome heads
receive the auxiliary gradient. The optional consumer KL also differentiates
through a local posterior whose parameters, deterministic input, and target are
frozen, allowing only the predicted embedding to receive its gradient.
"""
