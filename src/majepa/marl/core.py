...
from ..replay_codec import compact_spaces, pack_entries, unpack_entries, slim_behavior
...
from ..training.self_fed import self_fed_losses, mixed_posterior_kl, frozen_posterior, posterior_logits
from ..training.interface_diagnostics import availability_metrics
from ..training.gradient_routing import outcome_input_losses
...
class TeamAxisAdapter:
    ...
    def ext_space(self):
        ...
        compact = self.config.get("compact_replay", False)
        if compact:
            dyn = self.config.dyn.parallel_transformer
            local_spaces = compact_spaces(local_spaces, int(dyn.stoch), int(dyn.classes))
            spaces = dict(local_spaces)
        ...
        if self.two_branch_replay:
            if compact:
                replay_view = slim_behavior(replay_view)
            spaces.update(
                {
    ...
    def policy(self, carry, obs, mode="train"):
        ...
        if self.config.get("compact_replay", False):
            dyn = self.config.dyn.parallel_transformer
            output = pack_entries(output, int(dyn.stoch), int(dyn.classes))
        return (
    ...
    def train(self, carry, data):
        ...
        if self.config.get("compact_replay", False):
            dyn = self.config.dyn.parallel_transformer
            data = unpack_entries(data, int(dyn.stoch), int(dyn.classes))
        ...
        if "replay" in output:
            if self.config.get("compact_replay", False):
                output = dict(output, replay=pack_entries(
                    output["replay"], int(dyn.stoch), int(dyn.classes)))
            output = dict(
                output,
...
class MARLCore(TeamAxisAdapter, LocalAgent):
    ...
    def __init__(self, obs_space, act_space, config):
        ...
        self.ctde_mask_calibration = False
        ...
        self.ctde_soft_liveness = False
        self.ctde_recurrent_jepa_replacement = bool(
            marl.ctde.get("recurrent_jepa_replacement", False)
        )
        if self.ctde_recurrent_jepa_replacement:
            if marl.ctde.multistep_jepa.enabled or not self.ctde_self_fed_enabled:
                raise ValueError("Recurrent replacement requires direct JEPA disabled and self-fed enabled")
        ...
        if not math.isfinite(self.ctde_posterior_alignment_scale) or self.ctde_posterior_alignment_scale < 0:
    ...
    def _ctde_replay_losses(
        ...
        if self.joint_factual_gradient or self.joint_outcome_gradient:
            stopped = jax.lax.stop_gradient(source_state)
            source_state = stopped + self.joint_prediction_scale * (source_state - stopped)
        ...
        joint_args = (cache, source_state, source_action, source_present,
                      source_alive, reset, training)
        history_prediction = None
        # Snapshot only for the input-only outcome pass; ordinary RNG draws
        # and joint/head gradients follow the original execution order.
        outcome_rng = (nj.context().seed, tuple(nj.context().reserve)) if (
            self.joint_outcome_gradient and not nj.creating()) else None
        if self.joint_factual_gradient:
            _, prediction, snapshots = self.ctde_joint.sequence(
                *joint_args, stop_state_gradient=False)
        elif (self.config.marl.ctde.get("factual_jepa_history_gradient", False)
                and not nj.creating()):
            # Identical dropout draws for both passes. The ordinary pass keeps
            # every existing objective's local-state boundary unchanged.
            seed = nj.seed()
            params = {key: value for key, value in nj.context().items()
            ...
            _, (_, history_outputs, _) = sequence(
                frozen, *frozen_args, stop_state_gradient=False,
                seed=seed, create=False, modify=False)
        ...
        if outcome_rng is not None:
            input_losses = outcome_input_losses(
                self, joint_args, grouped_reward[:, 1:], continuation,
                grouped_alive[:, 1:], outcome_rng,
                ema_target if self.outcome_jepa_scale else None)
            input_reward, input_continuation, input_alive = input_losses[:3]
            if self.outcome_input_scale:
                reward_loss += input_reward - jax.lax.stop_gradient(input_reward)
                continuation_loss += input_continuation - jax.lax.stop_gradient(input_continuation)
                alive_loss += input_alive - jax.lax.stop_gradient(input_alive)
            if self.outcome_jepa_scale:
                input_jepa = input_losses[3]
                # Scale only the local-input Jacobian of the existing cosine
                # objective. Joint parameters, EMA targets and head updates
                # remain on their original paths; no extra model pass is used.
                embedding_loss += self.outcome_jepa_scale * (
                    input_jepa - jax.lax.stop_gradient(input_jepa))
        alive_valid = (
        ...
        metrics = {
            "ctde/outcome_jepa_input_scale": jnp.asarray(self.outcome_jepa_scale),
        ...
        if self.ctde_posterior_alignment_scale > 0:
            if self.joint_predicted_gradient:
                logits = posterior_logits(
                    self.dyn, folded_prediction, folded_deter,
                    history_gradient=self.joint_prediction_scale,
                    parameter_gradient=self.joint_prediction_scale)
            else:
    ...
    def _ctde_self_fed_report(
        ...
        def transition(state, action):
            ...
            if self.ctde_compare_mask_heads:
                local_output = self.actmask(self.feat2tensor(next_features), 1)
        ...
        if self.ctde_enabled:
            ...
            return (
                ...
                first,
                {
                    ...
                    **({"policy_logit": self.team.unfold_batch(first["logit"][:, 0])}
                       if self.policy_latent_readout == "probabilities" else {}),
                },
            )
    ...
    def imagine_with_aux(self, starts, policy, horizon, training, context=None):
        ...
        if self.policy_latent_readout == "probabilities":
            grouped_carry = dict(grouped_carry, _actor_logit=nn.cast(context["policy_logit"]))
        ...
        def transition(state, _):
            ...
            if self.policy_latent_readout == "probabilities":
                local_features["logit"] = local_carry["_actor_logit"]
            ...
            distribution = self.policy_distribution(
                self.policy_features_tensor(folded_features),
                1,
            ...
            if self.policy_latent_readout == "probabilities":
                next_carry = dict(next_carry, _actor_logit=next_features["logit"])
            ...
            if self.ctde_imagination_mask_source == "local":
        ...
        if self.policy_latent_readout == "probabilities":
            local_carry = {key: value for key, value in local_carry.items() if key != "_actor_logit"}
