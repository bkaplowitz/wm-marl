#!/usr/bin/env python3
"""Actor 2x256 on the unchanged one-environment localmask reference, after simplifications."""
...
ROOT = Path("/workspace/majepa_actor2x256_20260918")
GROUP = "ma-jepa-localmask-actor2x256-20260918"
PREDECESSOR = Path("/workspace/majepa_simplification_20260918/queue.json")
...
ARMS = {name: (0.5, HISTORICAL_DECAY) for name in ("actor2x256",)}
PLAN = [
    (arm, map_name, seed)
    for arm in ARMS
    for seed in (0, 1, 2)
...
def job_args(run: Spec, gpu: int):
    ...
    args.screen_label = (
        f"Localmask simplification {run.arm} / WM4096c64 / Actor2x256 / "
...
def resolve(run: Spec, gpu: int):
    ...
    common = {
        ...
        "agent.policy.units": 256,
        "agent.policy.layers": 2,
        "agent.marl.ctde.joint.temporal_layers": 12,
        "replay_context": 192,
...
def initialize():
        ...
        jobs.append(
            {
                ...
                "name": run.name,
                "status": "pending",
                "phase": "localmask_actor_capacity",
                "spec": asdict(run),
    ...
    atomic_json(
        ROOT / "queue.json",
        {
            ...
            "authorization": "User requested actor 2x256 queued behind simplifications; same maps and seeds.",
            "protocol": "One env. Only actor hidden layers/width change: 3x512 to 2x256. All heads retained, temporal layers12 and replay context192 unchanged. Plain localmask, no shared-mask supervision. Fixed total 50k/100k records, 5k prefill, train_ratio128, local/joint LR1e-4, entropy.003, mixed independent replay, start4, BPTT2, final100. Original env1 driver path retained.",
            "arms": list(ARMS),
