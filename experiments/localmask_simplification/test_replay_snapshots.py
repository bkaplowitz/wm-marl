...
class Replay(DualViewReplay):
    ...
    def populate(self, count):
        while self.records < count:
            ...
            self.add(
                {
                    "observation": np.array([record], np.float32),
                    "reward": np.float32(record % 11),
                    "action": np.int32(record % 7),
    ...
    def writeback(self, version):
        ...
        for chunk in list(self.chunks.values()):
            if chunk.length:
                ...
                self.update(
                    {"stepid": stepids, "dyn/pair": values, "dyn/stoch": values}
                )
...
def paired(replay, report=False):
    ...
    return _with_prefixed_batch(
        ...
        source("report" if report else "train_behavior"),
        "_behavior_replay/",
    )
...
def test_preserves_eager_first_window_and_read_before_writeback():
    ...
    for prefix in ["", "_behavior_replay/"]:
        np.testing.assert_array_equal(
            np.asarray(first[prefix + "observation"])[:, 0, 0], 0
        )
    assert replay.reads[-2:] == [
        ("train_world", 5000, 4745),
        ("train_behavior", 5000, 4745),
    ]
...
def test_reports_delays_and_trace_do_not_change_training_batches(tmp_path):
    def execute(extra_reports):
        ...
        for update in range(8):
            ...
            result.append(
                (
                    sampled_at,
                    replay_batch_fingerprints(data),
    ...
    assert (tmp_path / "traceFalse.jsonl").read_text() == (
        tmp_path / "traceTrue.jsonl"
    ).read_text()
...
def test_fingerprints_ignore_only_storage_ids_and_detect_history_changes():
    data = {
        "observation": np.array([[1, 2]], np.float32),
...
def test_matches_actual_legacy_transport_with_serialized_prefetch_reads():
    ...
    try:
        for update in range(8):
            ...
            assert replay_batch_fingerprints(previous) == replay_batch_fingerprints(
                current
            )
