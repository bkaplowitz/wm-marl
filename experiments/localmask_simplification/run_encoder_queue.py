#!/usr/bin/env python3
"""Independent encoder ablations on the one-environment localmask reference."""
...
ROOT = Path("/workspace/majepa_encoder_ablations_20260918")
GROUP = "ma-jepa-localmask-encoder-20260918"
PREDECESSOR = Path("/workspace/majepa_actor2x256_20260918/queue.json")
...
ARMS = {name: (0.5, HISTORICAL_DECAY) for name in ("encoder2x1024", "nosymlog")}
PLAN = [
    (arm, map_name, seed)
    for arm in ARMS
    for seed in (0, 1, 2)
    for map_name in ("2s3z", "8m", "3s_vs_4z")
]
...
class Spec(tg.Spec):
    ...
    def configuration_flags(self):
        ...
        decay_indices = [
            i for i, value in enumerate(flags) if value == "--replay.recency_decay"
        ]
        ...
        mix_indices = [
            i for i, value in enumerate(flags) if value == "--replay.world_uniform_mix"
        ]
        if not mix_indices:
        ...
        return tuple(flags) + (
            ...
            "--replay.behavior_uniform_mix",
            repr(float(self.behavior_uniform_mix)),
            "--agent.marl.ctde.imagination_mask_source",
            self.mask_source,
            "--agent.marl.ctde.compare_mask_heads",
            str(self.arm != "nojointmask"),
            "--agent.loss_scales.ctde_multistep_jepa_cosine",
            str(self.direct_cosine_scale),
            "--agent.simplification.joint_mask",
            str(self.arm != "nojointmask"),
            "--agent.simplification.local_outcomes",
            str(self.arm != "nolocaloutcomes"),
            "--agent.rewhead.units",
            str(256 if self.arm == "heads256" else 512),
            "--agent.conhead.units",
            str(256 if self.arm == "heads256" else 512),
            "--agent.maskhead.units",
            str(256 if self.arm == "heads256" else 512),
            "--agent.enc.simple.layers",
            str(2 if self.arm == "encoder2x1024" else 3),
            "--agent.enc.simple.symlog",
            str(self.arm != "nosymlog"),
        )
...
def job_args(run: Spec, gpu: int):
    ...
    root_label = "uniform WM" if run.world_uniform_mix == 1.0 else "50/50 WM"
    behavior_label = (
        "uniform PPO roots"
...
def resolve(run: Spec, gpu: int):
    ...
    common = {
        ...
        "agent.enc.simple.layers": 2 if run.arm == "encoder2x1024" else 3,
        "agent.enc.simple.units": 1024,
        "agent.enc.simple.symlog": run.arm != "nosymlog",
    ...
    for phase, command in commands(run, gpu).items():
        parsed, rest = elements.Flags(configs=["smac_vector", "ma_jepa"]).parse_known(
            command[3:]
        )
        ...
        if mismatches:
            raise RuntimeError(
                f"{run.name}/{phase}: resolved config mismatch: {mismatches}"
            )
        result[phase] = {
            "command": command,
            "config": current,
...
def storage_check():
    ...
    with (ROOT / "storage.lock").open("a") as lock:
        ...
        if cached.exists():
            ...
            if (
                time.time() - record["at"] < 300
                and record["headroom_bytes"] > 40_000_000_000
            ):
                return record
        used = int(
        ...
        available = int(
            os.statvfs("/workspace").f_bavail * os.statvfs("/workspace").f_frsize
        )
...
def initialize():
    ...
    if fingerprint != EXPECTED_SHA:
        raise RuntimeError(
            f"source fingerprint mismatch: {fingerprint} != {EXPECTED_SHA}"
        )
        ...
        jobs.append(
            {
                ...
                "name": run.name,
                "status": "pending",
                "phase": "localmask_encoder_ablation",
                "spec": asdict(run),
    ...
    atomic_json(
        ROOT / "queue.json",
        {
            ...
            "authorization": "User requested separate encoder 2x1024 and observation symlog-off ablations behind actor2x256; same three maps and seeds.",
            "protocol": "One env. Each arm independently changes encoder depth OR observation symlog. All heads retained; actor remains 3x512; embedding stays1024. Plain localmask, no shared-mask supervision. Fixed total 50k/100k records, 5k prefill, train_ratio128, local/joint LR1e-4, entropy.003, mixed independent replay, start4, BPTT2, final100. Original env1 driver path retained.",
            "arms": list(ARMS),
...
def run_job(index: int, gpu: int):
    ...
    def profile(_args, _run, _env):
        ...
        if json.loads(json.dumps(actual)) != expected:
            raise RuntimeError(
                f"saved and actual resolved configurations differ for {run.name}"
            )
        return actual
    ...
    with lock_path.open("a") as lock:
        ...
        while True:
            ...
            if blocked or compute_pids:
                atomic_json(
                    ROOT / "workers" / f"gpu{gpu}.waiting.json",
                    {
